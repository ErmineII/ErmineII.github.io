...
console.log = (args)=>{myconsole.log(args); oldLog.apply(console, arguments);};


var oldErr = console.error;
console.error = (args)=>{myconsole.error(args); oldErr.apply(console, arguments);};

myconsole.inputs = []
myconsole.inputoffset = 0

myconsole.inp.onkeypress = function(key){
  myconsole.inputs = myconsole.inputs.map(
    (x)=>{return x.replace(/^ *$/,"")} );
  switch (key.keyCode){
    case 13:
      if (key.shiftKey){
        myconsole.inputoffset -= 1;
        myconsole.inp.value = myconsole.inputs[
          myconsole.inputs.length - myconsole.inputoffset] || myconsole.inp.value;
        break;
      }
      ...
      waiting = false;
      myconsole.inputhandler &&
        myconsole.inputhandler(myconsole.input); 
      myconsole.inputoffset = 0;
      break;
    case 32:
